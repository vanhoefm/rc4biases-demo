/*
 *  Modified code of R-3.1.1 to simulate binomial distributions for
 *  very large N.
 *
...
uint64_t rbinom(uint64_t nin, long double pp)
{
    ...
    static long double c, fm, npq, p1, p2, p3, p4, qn;
    static long double xl, xll, xlr, xm, xr;

    static long double psave = -1.0;
    static uint64_t nsave = -1;
    static uint64_t m;

    long double f, f1, f2, u, v, w, w2, x, x1, x2, z, z2;
    long double p, q, np, g, r, al, alv, amaxp, ffm, ynorm;
    uint64_t i, ix, k, n;
    ...
    */
	...
	if (np < 30.0) {
	    ...
	    fm = m;
	    npq = np * q;
	    p1 = (uint64_t)(2.195 * sqrt(npq) - 4.6 * q) + 0.5;
	    xm = fm + 0.5;
	    xl = xm - p1;
	    xr = xm + p1;
    ...
    repeat {
	  ...
	  if (u > p3) {	/* right tail */
	      ix = (uint64_t)(xr - log(v) / xlr);
	      if (ix > n)
		  continue;
	      v = v * (u - p3) * xlr;
	  } else {/* left tail */
	      ix = (uint64_t)(xl + log(v) / xll);
	      if (ix < 0)
		  continue;
	      v = v * (u - p2) * xll;
	  }
